...
if kategori == "Pencarian Akar":
    ...
    if metode in ["Bi Section", "False Position"]:
        x_true = st.number_input("Nilai Sebenarnya", value=24.0)
        tol = st.number_input("Toleransi (%)", value=0.1)
        xl = st.number_input("Batas bawah $(x_l)$", value=18.0)
        xu = st.number_input("Batas atas $(x_u)$", value=37.0)
    elif metode == "Fixed Point":
        x_true = st.number_input("Nilai Sebenarnya", value=24.0)
        tol = st.number_input("Toleransi (%)", value=0.1)
        x0 = st.number_input("Nilai awal $(x_0)$", value=20.0)
        ...
        x_true = st.number_input("Nilai Sebenarnya", value=24.0)
        tol = st.number_input("Toleransi (%)", value=0.1)
        x0 = st.number_input("$(x_0)$", value=18.0)
        x1 = st.number_input("$(x_1)$", value=37.0)
...
# -- SPL --
    ...
    vector_text = st.text_input("Matriks B (pisah koma)", "8, 13, 7")
...
# -- Interpolasi --
    ...
    if metode in [
        ...
        x0_val = st.number_input("$x_0$ (titik acuan)", value=1.0)
...
# -- ODE --
    ...
    fungsi = st.text_input(
        "$\\frac{dy}{dx} = F(x_i, y_i)$", "-2*x**3 + 12*x**2 - 20*x + 8.5"
    )
...
if st.button("Hitung"):
    ...
    if metode == "Bi Section":
        ...
        df1, steps, akar, err = solver.solve()
        ...
        df1, steps, roots, err = solver.solve()
        akar = roots
    ...
    # -- SPL --
        ...
        df1, steps, result, err = solver.solve()
    elif metode == "Jacobi":
        A = parse_matrix(matrix_text)
        B = parse_vector(vector_text)
    ...
    # Error-only
    if err is not None and df1 is None:
        st.warning(err)
        st.stop()

    # Tabel
    if (df1 is not None) or (df1 is not None and df2 is not None):
        ...
        if kategori == "ODE (Persamaan Diferensial)":
            c1, c2 = st.columns(2)
            with c1:
    ...
    if kategori == "Pencarian Akar":
        ...
        if metode == "Polynomial Factorization" and roots is not None:
        ...
        elif isinstance(df1, pd.DataFrame) and not df1.empty and akar is not None:
            ...
            with tab2:
                plot_convergence(df1, metode)
            with tab3:
                plot_error(df1, metode)
            with tab4:
                plot_iteration_comparison(df1)
    ...
    # TODO: for ODE, don't display the result
    if metode == "Polynomial Factorization" and roots is not None:
        render_success_result(
